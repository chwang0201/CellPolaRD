...
//! \cond [init fun] \endcond
	...
	while (it.isNext())
	{
		...
		Old.template get<V>(key) = 1.0;
        Old.template get<W>(key) = 0.0;
...
int main(int argc, char* argv[])
{
    ...
    // Diffusion constant for specie W
    double dw = 1*1e-5;
	...
	for (size_t i = 0; i < timeSteps; ++i)
	{
		...
		while (it.isNext())
		{
			...
			New.get<U>(key) = Old.get<U>(key) + uFactor * (
										...
										Old.get<U>(key.move(y,-1)) +
										-4.0*Old.get<U>(key)) +
										- deltaT * Kon * Old.get<U>(key) * Old.get<V>(key) +
										  deltaT * Koff * Old.get<W>(key);

			// update V based on Eq 2
		...
		if (i % 100 == 0)
		{
			Old.write("output_CellPolaRD",count);
			count++;
		}
	}
